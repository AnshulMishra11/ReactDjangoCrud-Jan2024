...
const Create = () => {
  ...
  const {handleSubmit, control} = useForm({defaultValues:defaultValues})
...

    {
      ...
      const StartDate = data.start_date ? Dayjs(data.start_date["$d"]).format("YYYY-MM-DD") : null;
      const EndDate = data.end_date ? Dayjs(data.end_date["$d"]).format("YYYY-MM-DD") : null;
